...
public abstract class ClassUtils {
    ...
    static {
        ...
        for (Iterator it = ClassUtils.primitiveWrapperTypeMap.values().iterator(); it.hasNext();) {
            Class primitiveClass = (Class) it.next();
            ClassUtils.primitiveTypeNameMap.put(primitiveClass.getName(), primitiveClass);
        }
    }
    ...
    public static boolean isPresent(String className) {
        try {
            ...
            if (ClassUtils.logger.isDebugEnabled()) {
                ClassUtils.logger.debug("Class [" + className + "] or one of its dependencies is not present: " + ex);
            }
            return false;
        }
    }
    ...
    public static Class forName(String name, ClassLoader classLoader) throws ClassNotFoundException {
        ...
        if (name.endsWith(ClassUtils.ARRAY_SUFFIX)) {
            ...
            String elementClassName = name.substring(0, name.length() - ClassUtils.ARRAY_SUFFIX.length());
    ...
    public static Class resolvePrimitiveClassName(String name) {
        ...
        if (name != null && name.length() <= 8) {
            ...
            result = (Class) ClassUtils.primitiveTypeNameMap.get(name);
        }
        return result;
    }
     ...
     * @param className the className to getObject the short name for
    ...
    public static String getShortName(String className) {
        ...
        int nameEndIndex = className.indexOf(ClassUtils.CGLIB_CLASS_SEPARATOR_CHAR);
        if (nameEndIndex == -1) {
            nameEndIndex = className.length();
        }
        String shortName = className.substring(lastDotIndex + 1, nameEndIndex);
        shortName = shortName.replace(ClassUtils.INNER_CLASS_SEPARATOR_CHAR, ClassUtils.PACKAGE_SEPARATOR_CHAR);
        return shortName;
    }

    /**
     * Get the class name without the qualified package name.
     * @param clazz the class to getObject the short name for
     * @return the class name of the class without the package name
     */
    ...
    public static int getMethodCountForName(Class clazz, String methodName) {
        ...
        for (int i = 0; i < ifcs.length; i++) {
            count += ClassUtils.getMethodCountForName(ifcs[i], methodName);
        }
    ...
    public static boolean hasAtLeastOneMethodWithName(Class clazz, String methodName) {
        ...
        Class[] ifcs = clazz.getInterfaces();
        for (int i = 0; i < ifcs.length; i++) {
            if (ClassUtils.hasAtLeastOneMethodWithName(ifcs[i], methodName)) {
                return true;
            }
        }
        if (clazz.getSuperclass() != null) {
            return ClassUtils.hasAtLeastOneMethodWithName(clazz.getSuperclass(), methodName);
        }
        return false;
    }
    ...
    public static boolean isAssignable(Class targetType, Class valueType) {
        ...
        return (targetType.isAssignableFrom(valueType) ||
                targetType.equals(ClassUtils.primitiveWrapperTypeMap.get(valueType)));
    }
